...
Betas <- c(top1=100000,top2=75000,top3=25000,top4=5000,top5=500,top6=0)
...
for(i in 1:10) { 
        for(j in 1:30){
                Zeta[[idx]] <- rdirichlet(1,rep(0.1,6))
                idx <- idx + 1
        }
}
...
for(i in 1:10) { 
        for(j in 1:30){
                k[i,j] <- rmultinom(1,6,Zeta[[idx]])[1]
                idx <- idx + 1
        }
        k[i,k[i,] == 0] <- 1
        k[i,k[i,] == 4] <- 2
